...
# Global dictionaries to track trades and orders
open_trades = {}
pending_orders = {}
symbol = "BTCUSD"
...
class TradingTool:
    def __init__(self):
        self.account = {
            ...
            "login": int(os.getenv("MT5_LOGIN", "79555324")),
            "password": os.getenv("MT5_PASSWORD", getpass("Enter MT5 password: ")),
        }
        self.magic_number = 12345  # Unique identifier for script-generated trades
    ...
    def connect_to_account(self):
        ...
        for attempt in range(1, retries + 1):
            if mt5.initialize(
                ...
                logging.info(
                    f"Connected to account {self.account['login']} on server {self.account['server']}."
                )
                return True
            logging.error(
                f"Failed to connect to account {self.account['login']}. Attempt {attempt}/{retries}. Error: {mt5.last_error()}"
            )
    ...
    def validate_input(
        ...
        """Validate user input based on type, range, and optional values."""
        if values is None:
            values = []
        while True:
    ...
    def initialize_trade_tracking(self):
        """Fetch and track open trades and pending orders."""
    ...
    def place_order(self):
        ...
        direction = self.validate_input("Enter direction (buy/sell): ", str).lower()
        if direction not in ["buy", "sell"]:
            print("Invalid direction. Please enter 'buy' or 'sell'.")
            return

        lot_size = self.validate_input(
            "Enter lot size (e.g., 0.01): ", float, min_value=0.01
        )
        tp = self.validate_input(
            "Enter Take Profit (optional, press Enter to skip): ", float, optional=True
        )
        sl = self.validate_input(
    ...
    def update_order(self):
        """Update an open trade's TP/SL."""
        if not open_trades:
            print("No open trades to update.")
            return

        ticket = self.validate_input(
            "Enter the ticket ID of the trade to update: ", int
        )
        ...
        tp = self.validate_input(
            "Enter new Take Profit (optional, press Enter to skip): ",
            float,
            optional=True,
        )
        sl = self.validate_input(
            "Enter new Stop Loss (optional, press Enter to skip): ",
            float,
            optional=True,
        )
    ...
    def close_order(self):
        ...
        ticket = self.validate_input("Enter the ticket ID of the trade to close: ", int)
        if ticket not in open_trades:
            print("Invalid ticket ID.")
            return
    ...
    def close_partial_position(self):
        ...
        perc_close = self.validate_input(
        ...
        if result.retcode == mt5.TRADE_RETCODE_DONE:
            ...
            logging.error(
                f"Failed to partially close position. Error: {result.comment}"
            )

    def show_all_open_trades(self):
        ...
        if not open_trades:
            ...
            for ticket, trade in open_trades.items():
                print(
                    f"Ticket: {ticket}, Symbol: {trade['symbol']}, Lot Size: {trade['lot_size']}, "
                    f"TP: {trade['tp']}, SL: {trade['sl']}, Type: {trade['type']}"
                )
    ...
    def place_pending_order(self):
        ...
        order_type = self.validate_input(
            "Enter order type (buy_limit/sell_limit/buy_stop/sell_stop): ",
            str,
            values=["buy_limit", "sell_limit", "buy_stop", "sell_stop"],
        ).lower()

        lot_size = self.validate_input(
            "Enter lot size (e.g., 0.01): ", float, min_value=0.01
        )
        price = self.validate_input("Enter price for the pending order: ", float)
        tp = self.validate_input(
            "Enter Take Profit (optional, press Enter to skip): ", float, optional=True
        )
        sl = self.validate_input(
    ...
    def update_pending_order(self):
        """Update a pending order's price, TP, or SL."""
        if not pending_orders:
            print("No pending orders to update.")
            return

        ticket = self.validate_input(
            "Enter the ticket ID of the pending order to update: ", int
        )
        if ticket not in pending_orders:
            print("Invalid ticket ID.")
            return

        price = self.validate_input("Enter new price: ", float)
        tp = self.validate_input(
            "Enter new Take Profit (optional, press Enter to skip): ",
            float,
            optional=True,
    ...
    def remove_pending_order(self):
        ...
        ticket = self.validate_input(
            "Enter the ticket ID of the pending order to remove: ", int
        )
        if ticket not in pending_orders:
            print("Invalid ticket ID.")
            return
    ...
    def show_all_pending_trades(self):
        ...
        if not pending_orders:
            ...
            for ticket, order in pending_orders.items():
                print(
                    f"Ticket: {ticket}, Symbol: {order['symbol']}, Lot Size: {order['lot_size']}, "
                    f"Price: {order['price']}, Type: {order['type']}, TP: {order['tp']}, SL: {order['sl']}"
                )

    def main(self):
        """Main menu for the trading tool."""
        self.connect_to_account()
        self.initialize_trade_tracking()
        ...
        while True:
            ...
            if choice == "1":
                ...
                self.initialize_trade_tracking()
                self.remove_pending_order()
            elif choice == "9":
                self.initialize_trade_tracking()
                self.show_all_pending_trades()
            elif choice == "0":
